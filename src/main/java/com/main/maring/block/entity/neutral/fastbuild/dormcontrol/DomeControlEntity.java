package com.main.maring.block.entity.neutral.fastbuild.dormcontrol;
...
public class DomeControlEntity extends BlockEntity{
	...
	public void servertick(ServerLevel serverLevel) {
		...
		if(!serverLevel.getBlockState(this.getBlockPos().above()).isAir()) return;
		if(this.getO2()[0]<5) {
			AAirManager manager = AAirManager.getOrCreate(serverLevel);
			serverLevel.setBlockAndUpdate(this.getBlockPos().above(), BlockRegister.A_AIR.get().defaultBlockState().setValue(BlockAAIR.STATE, AAirState.DEACTIVATE));
			manager.add(this.getBlockPos().above(),AAirState.DEACTIVATE);
		}else if(this.getO2()[0]>10){
			AAirManager manager = AAirManager.getOrCreate(serverLevel);
			serverLevel.setBlockAndUpdate(this.getBlockPos(),BlockRegister.A_AIR.get().defaultBlockState().setValue(BlockAAIR.STATE, AAirState.ACTIVATE));
			manager.add(this.getBlockPos().above(),AAirState.ACTIVATE);
		}
	}

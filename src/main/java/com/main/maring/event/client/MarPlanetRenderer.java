...
@OnlyIn(Dist.CLIENT)
public class MarPlanetRenderer extends DimensionSpecialEffects {

	public static boolean shouldnotRenderClouds = true;
	public static boolean shouldRenderSea = false;
	public static boolean shouldRenderIce = true;

	private static int SPACE_HEIGHT = 384;
	@Nullable
	...
	private static final ResourceLocation MAR_PLANET = new ResourceLocation(Maring.MODID,"textures/environment/mar_planet.png");
	private static final ResourceLocation MAR_PLANET_ICE = new ResourceLocation(Maring.MODID,"textures/environment/mar_planet_ice.png");
	private static final ResourceLocation MAR_PLANET_SEA = new ResourceLocation(Maring.MODID,"textures/environment/mar_planet_sea.png");
	private static final ResourceLocation MAR_PLANET_ATMOSPHERE = new ResourceLocation(Maring.MODID,"textures/environment/mar_planet_atmosphere.png");


	public MarPlanetRenderer(float cloudLevel, boolean hasGround, SkyType p_108868_, boolean forceBrightLightmap, boolean constantAmbientLight) {
    ...
    public boolean renderClouds(ClientLevel level, int ticks, float partialTick, PoseStack poseStack, double camX,
    	...
    	if(this.minecraft.player.getY()>SPACE_HEIGHT||shouldnotRenderClouds) {
	...
	public Vec3 getBrightnessDependentFogColor(Vec3 colour, float brightness) {
		if(this.minecraft.player.getY()>SPACE_HEIGHT){
    ...
    public boolean renderSky(ClientLevel level, int ticks, float partialTick, PoseStack poseStack, Camera camera,
		  ...
		  if (!isFoggy) {
		      ...
		      if (fogtype != FogType.POWDER_SNOW && fogtype != FogType.LAVA && !this.doesMobEffectBlockSky(camera)) {
		        ...
		        if (this.minecraft.player.getY()<=SPACE_HEIGHT && afloat != null) {
		        ...
		        poseStack.pushPose();
			     ...
			     // 获取星星的亮度并绘制星星
			     float f10 = this.minecraft.player.getY()>SPACE_HEIGHT ? 1f : level.getStarBrightness(partialTick) * f11 * 1.5f;
			     if (f10 > 0.0F) {
				...
				if(this.minecraft.player.getY()>SPACE_HEIGHT) {
					f12 = 100.0F;
					RenderSystem.setShader(GameRenderer::getPositionTexShader);
					RenderSystem.setShaderTexture(0, MAR_PLANET);
					...
					if(shouldRenderSea){
						RenderSystem.setShaderTexture(0, MAR_PLANET_SEA);
						bufferbuilder.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_TEX);
						bufferbuilder.vertex(matrix4f1, -f12, 100.0F, -f12).uv(0.0F, 0.0F).endVertex();
						bufferbuilder.vertex(matrix4f1, f12, 100.0F, -f12).uv(1.0F, 0.0F).endVertex();
						bufferbuilder.vertex(matrix4f1, f12, 100.0F, f12).uv(1.0F, 1.0F).endVertex();
					...
					if(shouldRenderIce){
						RenderSystem.setShaderTexture(0, MAR_PLANET_ICE);
						bufferbuilder.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_TEX);
						bufferbuilder.vertex(matrix4f1, -f12, 100.0F, -f12).uv(0.0F, 0.0F).endVertex();
						bufferbuilder.vertex(matrix4f1, f12, 100.0F, -f12).uv(1.0F, 0.0F).endVertex();
						bufferbuilder.vertex(matrix4f1, f12, 100.0F, f12).uv(1.0F, 1.0F).endVertex();
					...
					if(!shouldnotRenderClouds){
						RenderSystem.setShaderTexture(0, MAR_PLANET_ATMOSPHERE);
						RenderSystem.enableBlend();
						bufferbuilder.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_TEX);
						bufferbuilder.vertex(matrix4f1, -f12, 100.0F, -f12).uv(0.0F, 0.0F).endVertex();
						bufferbuilder.vertex(matrix4f1, f12, 100.0F, -f12).uv(1.0F, 0.0F).endVertex();
		        ...
		        if (d0 < 0.0D&&this.minecraft.player.getY()<=SPACE_HEIGHT) {
		           poseStack.pushPose();
	   ...
	   private void createLightSky() {
	    ...
	    {
	    	if(this.minecraft.player.getY()>200||shouldnotRenderClouds) {
	    		return true;
	    	};
	    	return false;	
	    	/*
